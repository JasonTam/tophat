...
config = Config('tiefrex/config/fit_config-integ.py')
...
def run():
    ...
    if config.get('ckpt_upload_s3_uri'):
        # TODO: write this
        from lib_cerebro_py.aws.aws_s3_object import AwsS3Object
        for p in glob.glob(os.path.join(LOG_DIR, '**')):
            path_upload = os.path.join(
                config.get('ckpt_upload_s3_uri'),
                p.split(LOG_DIR)[-1].strip('/'))
            AwsS3Object(path_upload).upload_file(p)
