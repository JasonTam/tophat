...
if not os.path.exists(dir_model):
    os.mkdir(dir_model)
# Should be clean so we don't load any stray files
for f in glob.glob(os.path.join(dir_model, '*')):
    os.remove(f)


path_cats = os.path.join(BUCKET_EXPORTS, PREFIX, 'models-prod/cats_d.p')

# Prep for working around tf.train.latest_checkpoint with s3 directory
dir_ckpt = os.path.join(BUCKET_EXPORTS, PREFIX, 'models-prod/')
...
dir_export = os.path.join(BUCKET_EXPORTS, PREFIX, 'factors-prod')
